...
// Visualization.
DEFINE_bool(visualize_histogram_1D, true, "Visualize 1D histogram.");
DEFINE_bool(log_histogram_1D, false,
            "Log 1D histogram to file."
            " It logs the raw and smoothed histogram.");
DEFINE_bool(visualize_histogram_2D, true, "Visualize 2D histogram.");
...
//  std::cout << "ratioTangentialRadial_i: " << ratioTangentialRadial_i << std::endl;
//  std::cout << "min_elongation_ratio: " << min_elongation_ratio << std::endl << std::endl;
...
//        std::cout << "triangle_2d.rows: " << triangle_2d.rows << std::endl;
//        std::cout << "loop_end: " << loop_end << std::endl;
        if (j == loop_end) {
          // Last iteration.
...
bool Mesher::isNormalAroundAxis(const cv::Point3f& axis,
  ...
  std::cout << "std::fabs(normal.ddot(axis)): " << std::fabs(normal.ddot(axis)) << std::endl;
...
void Mesher::clusterPlanesFromMesh(
  ...
  std::cout << "Starting plane association..." << std::endl;
...
void Mesher::segmentPlanesInMesh(
  ...
  std::cout << "segment Planes In Mesh start!" << std::endl;
  ...
  std::cout << "mesh_3d_.getNumberOfPolygons(): " << mesh_3d_.getNumberOfPolygons() << std::endl;
  for (size_t i = 0; i < mesh_3d_.getNumberOfPolygons(); i++) {
    CHECK(mesh_3d_.getPolygon(i, &polygon)) << "Could not retrieve polygon.";
...
//      std::cout << "triangle_normal: " << triangle_normal << std::endl;
      ////////////////////////// Update seed planes ////////////////////////////
...
void Mesher::segmentHorizontalPlanes(std::vector<Plane>* horizontal_planes,
  ...
  std::cout << "z_components: " << std::endl << z_components << std::endl;
  ////////////////////////////// 1D Histogram //////////////////////////////////
  ...
  for (int peak_nr = 0;
    ...
    if (it != peaks.end()) {
      ...
      VLOG(10) << "Segmented an horizontal plane with:\n"
               ...
               << gtsam::DefaultKeyFormatter(plane_symbol.key())
               << "\n\t cluster id: " << cluster_id;
      std::cout << "Segmented an horizontal plane with:\n"
                << "\t distance: " << plane_distance << "\n\t plane id: "
                << gtsam::DefaultKeyFormatter(plane_symbol.key())
...
void Mesher::associatePlanes(const std::vector<Plane>& segmented_planes,
  ...
  if (planes.size() == 0) {
    ...
    std::cout << "No planes in backend, just copy the " << segmented_planes.size()
              << " segmented planes to the set of "
              << "backend planes, skipping data association." << std::endl;
    ...
    for (const Plane& segmented_plane : segmented_planes) {
      ...
      for (const Plane& plane_backend : planes) {
        ...
        if (plane_backend.geometricEqual(segmented_plane, normal_tolerance,
          ...
          std::cout << "backend_plane_index: " << backend_plane_index << std::endl;
...
void Mesher::updateMesh3D(
  ...
  std::cout << "mesh_2d_pixels size:   " << mesh_2d_pixels.size() << std::endl;
  std::cout << "mesh_2d_filtered size: " << mesh_2d_filtered.size() << std::endl;
  std::cout << "FLAGS_max_grad_in_triangle: " << FLAGS_max_grad_in_triangle << std::endl;
  ...
  if(mesh_2d != nullptr){
      cv::Mat mesh_2d_image = stereo_frame_ptr->getLeftFrame().img_.clone();
      cv::cvtColor(mesh_2d_image, mesh_2d_image, cv::COLOR_GRAY2RGB);
      for(size_t i = 0; i < mesh_2d->getNumberOfPolygons(); ++i) {
          Mesh2D::Polygon polygon;
          mesh_2d->getPolygon(i, &polygon);
          ...
          cv::Point2f p0 = cv::Point2f(0.0, 0.0);
          cv::Point2f p1 = cv::Point2f(0.0, 0.0);
          cv::Point2f p2 = cv::Point2f(0.0, 0.0);
          bool find_p0 = false;
          bool find_p1 = false;
          bool find_p2 = false;

          auto keypoints = stereo_frame_ptr->getLeftFrame().keypoints_;
          auto landmarks = stereo_frame_ptr->getLeftFrame().landmarks_;
          ...
          if (find_p0 && find_p1 && find_p2) {
              cv::circle(mesh_2d_image, p0, 2, cv::Scalar(0, 255, 255), -1);
              cv::circle(mesh_2d_image, p1, 2, cv::Scalar(0, 255, 255), -1);
              cv::circle(mesh_2d_image, p2, 2, cv::Scalar(0, 255, 255), -1);
              cv::line(mesh_2d_image, p0, p1, cv::Scalar(0, 0, 255));
              cv::line(mesh_2d_image, p0, p2, cv::Scalar(0, 0, 255));
              cv::line(mesh_2d_image, p1, p2, cv::Scalar(0, 0, 255));
      ...
      cv::imshow("mesh 2d image", mesh_2d_image);
//      cv::waitKey(500);
  }
